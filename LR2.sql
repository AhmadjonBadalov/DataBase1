-- 1. Ќайти максимальную цену товара (ListPrice) из
-- таблицы Production.Product.
SELECT MAX(ListPrice) AS MaxListPrice
FROM Production.Product;
-- 2. Ќайти минимальный вес (Weight) из таблицы Production.Product,
-- не учитывать пустые значени€.
SELECT MAX(Weight) AS MaxWeight 
FROM Production.Product 
WHERE Weight IS NOT NULL;
-- 3. Ќайти средний возраст мужчин и женщин из таблицы 
...
GROUP BY Gender;
-- 4. ¬ывести страны, где количество городов = 1, из таблицы 
-- Person.StateProvince (из выборки исключить пустые пол€).
SELECT CountryRegionCode
FROM Person.StateProvince
GROUP BY CountryRegionCode
HAVING COUNT(*) = 1;
-- 5. ¬ывести BusinessEntityID, LastReceiptDate и среднее по 
